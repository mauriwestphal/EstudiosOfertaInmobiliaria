...
// Load studies from KV via intake-worker
async function loadStudies() {
  ...
  try {
    ...
    if (result.success && Array.isArray(result.estudios)) {
      state.studies = result.estudios;
    } else {
      state.studies = [];
    }
      ...
      studiesTableBody.innerHTML = `
        <tr>
          <td colspan="6" style="text-align: center; padding: 40px;">
            <p class="status-rejected" style="color: var(--error); margin-bottom: 8px;">
              Error al cargar los estudios: ${error.message}
            </p>
            <p style="color: var(--text-muted); font-size: 0.85em;">
              Verifica que el intake-worker esté funcionando y que ADMIN_SECRET sea correcto.
            </p>
            <button onclick="loadStudies()" style="margin-top: 12px; padding: 8px 20px; background: var(--teal); color: white; border: none; border-radius: 6px; cursor: pointer;">
              Reintentar
            </button>
          </td>
        </tr>
...
function renderStudiesTable() {
  ...
  studiesTableBody.innerHTML = filteredStudies.map(study => `
    <tr data-study-id="${study.estudio_id}">
      ...
      <td>
        <div class="study-date">${study.createdAt ? new Date(study.createdAt).toLocaleDateString('es-CL') : '-'}</div>
      </td>
      <td>
...
function viewStudy(estudioId) {
  ...
  statusEl.textContent = study.estado === 'pendiente' ? 'Pendiente' :
    study.estado === 'aprobado' ? 'Aprobado' : 'Rechazado';
  statusEl.className = `study-status-modal status-${study.estado}`;

  // Show full JSON in viewer (include toda la data guardada en KV)
  jsonViewer.textContent = JSON.stringify(study.json || {}, null, 2);

  // Update contact info in details tab
...
// Approve study — call publish-worker
async function approveStudy(estudioId) {
  ...
  if (!confirm(`¿Aprobar el estudio ${study.codigo}?\n\nEsto publicará el JSON en GitHub y enviará un email al cliente.`)) {
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      // Also mark as approved in KV via intake-worker
      try {
        await fetch(`${INTAKE_WORKER_URL}/admin/approve`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            estudio_id: study.estudio_id,
        ...
        // Non-critical — publish already done
      }

      alert(`✅ Estudio ${study.codigo} publicado correctamente.\n\n${result.email_sent ? '📧 Email enviado al cliente.' : '⚠️ Email no enviado.'}`);
    } else {
...
async function rejectStudy(estudioId) {
  ...
  try {
    // Update in KV via intake-worker
